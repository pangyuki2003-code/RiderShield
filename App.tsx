...
import { generateEmergencySpeech } from './services/gemini';
...
export default function App() {
  ...
  const stopVideoRecording = () => { if (mediaRecorderRef.current?.state !== 'inactive') mediaRecorderRef.current?.stop(); };
  ...
  const triggerCrash = useCallback((severity: CrashSeverity = 'Medium') => {
    ...
    if (autoSaveEnabled) {
      startVideoRecording();
      setTimeout(stopVideoRecording, 15000);
    }
  ...
  const handleEmergencySequence = async () => {
    ...
    // Auto-dial primary sequence 1-4
    ...
    for (const contact of primaryContacts) {
      ...
      // In a real device environment, we'd trigger system level dialing
    }
    ...
    <div className="min-h-screen max-w-md mx-auto bg-[#FDFBF7] shadow-xl relative flex flex-col text-slate-800">
      ...
      <main className="flex-1 overflow-y-auto p-4 pb-24">
        ...
        {activeTab === 'dashcam' && (
          <DashcamView 
            ...
            onRecordToggle={() => isRecording ? stopVideoRecording() : startVideoRecording()} 
      ...
      {isCrashDetected && (
        <div className="fixed inset-0 z-50 bg-terracotta flex flex-col items-center justify-center p-8 text-white text-center">
          ...
          <button onClick={cancelCrash} className="w-full py-6 bg-white text-terracotta text-2xl font-bold rounded-2xl uppercase shadow-2xl active:scale-95 transition-all">
            {t.iamOk}
          </button>
        </div>
      )}
  ...
  return (
    <div className="space-y-4">
      <div className="relative aspect-[9/16] bg-black rounded-3xl overflow-hidden border-4 border-white shadow-2xl">
        ...
        <button onClick={onRecordToggle} className={`absolute bottom-12 left-1/2 -translate-x-1/2 p-6 rounded-full border-2 border-white/50 backdrop-blur-sm transition-all active:scale-90 ${isRecording ? 'bg-red-500 text-white' : 'bg-white/20 text-white'}`}>
          <VideoCameraIcon className="w-8 h-8" />
        </button>
      </div>
      <div className="bg-white p-4 rounded-2xl border border-sand flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-3">
          <BookmarkSquareIcon className="w-6 h-6 text-sage" />
          <span className="text-sm font-bold text-slate-700">{t.autoSave}</span>
        </div>
        <button onClick={onAutoSaveToggle} className={`w-12 h-6 rounded-full relative transition-colors ${autoSave ? 'bg-sage' : 'bg-slate-200'}`}>
          <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${autoSave ? 'right-1' : 'left-1'}`} />
        </button>
      </div>
    </div>
  );
...
const Contacts = ({ t, contacts, setContacts }: any) => {
  ...
  const add = () => { 
    if(name && phone) { 
      setContacts([...contacts, { id: Date.now().toString(), name, phone, priority: contacts.length + 1 }]); 
      setName(''); setPhone(''); 
    } 
  };
  ...
  const setPriority = (id: string, newP: number) => {
    const updated = contacts.map((c: any) => {
      if (c.id === id) return { ...c, priority: newP };
      return c;
    });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold">{t.emergencyContacts}</h2>
        <div className="bg-sand px-3 py-1 rounded-full flex items-center gap-1">
          <ArrowsUpDownIcon className="w-3 h-3 text-terracotta" />
          <span className="text-[10px] font-black uppercase text-terracotta">{t.dialOrder} (1-4)</span>
        </div>
      </div>
      ...
      <div className="space-y-3">
        {primaryContacts.map((c: any) => (
          <div key={c.id} className="bg-white p-4 rounded-2xl border-2 border-sage/20 flex justify-between items-center shadow-sm relative overflow-hidden">
            ...
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-sage text-white rounded-full flex items-center justify-center font-black text-sm">{c.priority}</div>
              <div><p className="font-bold text-slate-700">{c.name}</p><p className="text-xs text-slate-400">{c.phone}</p></div>
            </div>
            <div className="flex gap-2">
              {c.phone !== '999' && (
                <select 
                  className="bg-sand text-[10px] font-bold p-1 rounded outline-none" 
                  value={c.priority} 
                  onChange={(e) => setPriority(c.id, parseInt(e.target.value))}
                >
      ...
      {otherContacts.length > 0 && (
        <div className="space-y-3">
          <div className="flex items-center gap-4 py-2">
            <div className="h-px flex-1 bg-sand"></div>
            <span className="text-[10px] font-black uppercase tracking-widest text-slate-300">{t.otherContacts}</span>
            <div className="h-px flex-1 bg-sand"></div>
          </div>
          {otherContacts.map((c: any) => (
            <div key={c.id} className="bg-white/60 p-4 rounded-2xl border border-sand flex justify-between items-center shadow-sm">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 bg-sand rounded-full flex items-center justify-center font-bold text-slate-400 text-xs">#</div>
                <div><p className="font-bold text-slate-500">{c.name}</p><p className="text-[10px] text-slate-400">{c.phone}</p></div>
              </div>
              <div className="flex gap-2">
                <button 
                  className="bg-sand text-[10px] font-bold px-2 py-1 rounded"
                  onClick={() => setPriority(c.id, 4)}
                >
                  Move Up
                </button>
                <button onClick={() => setContacts(contacts.filter((x:any) => x.id !== c.id))} className="text-slate-200"><TrashIcon className="w-4 h-4"/></button>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="bg-sand/30 p-4 rounded-3xl border border-sand space-y-3">
        ...
        <div className="flex gap-2">
          <input type="text" placeholder="Name" value={name} onChange={e => setName(e.target.value)} className="flex-1 p-3 rounded-xl text-xs border-none shadow-sm outline-none" />
          <input type="tel" placeholder="Phone" value={phone} onChange={e => setPhone(e.target.value)} className="flex-1 p-3 rounded-xl text-xs border-none shadow-sm outline-none" />
          <button onClick={add} className="px-4 bg-slate-800 text-white rounded-xl active:scale-90 transition-all"><PlusIcon className="w-5 h-5"/></button>
        </div>
      </div>
    </div>
  );
    ...
    <div className="space-y-6 pb-24">
      <div className="flex flex-col items-center gap-4">
        <div className="relative w-28 h-28 rounded-3xl bg-white shadow-xl border-4 border-white overflow-hidden flex items-center justify-center group">
          {medicalId.personalPhoto ? (
            <img src={medicalId.personalPhoto} className="w-full h-full object-cover" />
          ) : (
        ...
        <div className="text-center">
          <h2 className="text-xl font-bold text-slate-800">{medicalId.fullName}</h2>
          <div className="flex items-center gap-1 justify-center mt-1">
            <CheckBadgeIcon className={`w-4 h-4 ${medicalId.isIcVerified ? 'text-sage' : 'text-slate-300'}`} />
            <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">{medicalId.isIcVerified ? t.verified : t.icVerify}</span>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="bg-white p-6 rounded-3xl border border-sand shadow-sm space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <MedicalField label="Blood Type" value={medicalId.bloodType} onChange={(v:any) => update('bloodType', v)} />
            <MedicalField label="Allergies" value={medicalId.allergies} onChange={(v:any) => update('allergies', v)} />
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-3xl border border-sand shadow-sm space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <MedicalField label="Brand / Model" value={medicalId.vehicleBrand} onChange={(v:any) => update('vehicleBrand', v)} />
            <MedicalField label="Vehicle Color" value={medicalId.vehicleColor} onChange={(v:any) => update('vehicleColor', v)} />
          </div>
        </div>
      </div>
    </div>
...
const MedicalField = ({ label, value, onChange }: any) => (
  <div className="space-y-1">
    <label className="text-[8px] uppercase font-black text-slate-400 tracking-widest">{label}</label>
    <input type="text" value={value} onChange={e => onChange(e.target.value)} className="w-full text-sm font-bold bg-transparent border-b border-sand focus:border-terracotta outline-none pb-1 transition-all" />
  </div>
);

const NavButton = ({ active, onClick, icon, label }: any) => (
  <button onClick={onClick} className={`flex flex-col items-center gap-1 transition-all flex-1 ${active ? 'text-terracotta scale-110' : 'text-slate-400'}`}>
    {icon}
